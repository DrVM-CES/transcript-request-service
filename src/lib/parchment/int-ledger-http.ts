...
async function rawBody(request:Request,limit:number,signal:AbortSignal){
  const declared=request.headers.get('content-length');
  if(request.headers.get('content-type')?.split(';')[0].trim().toLowerCase()!=='application/json'||(declared!==null&&(!/^\d+$/.test(declared)||Number(declared)>limit))||!request.body)throw new Error('invalid request');
  ...
  try{while(true){if(signal.aborted)throw new Error('deadline');const next=await reader.read();if(next.done)break;size+=next.value.length;if(size>limit)throw new Error('limit');parts.push(next.value);}if(signal.aborted)throw new Error('deadline');return Buffer.concat(parts);}
...
export function createIntLedgerHandlers(config:IntLedgerConfig,ledger:()=>Promise<IntLedger>,fetcher:typeof fetch=fetch){
  ...
  async function bounded(action:(signal:AbortSignal)=>Promise<Response>){
    ...
    catch(error){return reply(error instanceof LedgerConflict?409:503,{error:error instanceof LedgerConflict?'state_conflict':'temporarily_unavailable'});}
  ...
  const intake=(kind:'prepare'|'status')=>async(request:Request)=>{
    ...
    return bounded(async signal=>{
      ...
      const raw=await rawBody(request,4096,signal);let value:unknown;try{value=JSON.parse(new TextDecoder('utf-8',{fatal:true}).decode(raw));}catch{return reply(400,{error:'invalid_request'});}
  ...
  return {prepare:intake('prepare'),status:intake('status'),webhook:async(request:Request)=>{
    ...
    if(request.method!=='POST')return reply(405,{error:'method_not_allowed'});
    return bounded(async signal=>{
      const rawPayload=await rawBody(request,128_000,signal);
