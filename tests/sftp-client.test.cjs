...
test('only a completed upload reports success; errors are sanitized', async () => {
  Object.assign(process.env, { TRANSCRIPT_ENVIRONMENT: 'production', TRANSCRIPT_DELIVERY_MODE: 'live', PARCHMENT_SFTP_HOST: 'partner.test', PARCHMENT_SFTP_USERNAME: 'test', PARCHMENT_SFTP_PASSWORD: 'test-only' });
...
test('staging never opens SFTP even when live credentials are present', async () => {
  ...
  assert.deepEqual(await new ParchmentSFTPClient().uploadXML('<synthetic/>', 'test'), { success: false, error: 'SFTP_NOT_CONFIGURED' });
